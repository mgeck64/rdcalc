...
// <shift term> ::= <term> [ ( "+" | "-" ) <term> ]...
    ...
    for (;;) {
        if (lexer.peek_tok().id == token::add) {
            ...
            lval_num = basic_or_vector_op([](auto lval, auto rval, const error_context&) -> auto {
                return lval + rval;
...
// <term> ::= <factor> [ ( "*" | "/" | "%" | "." ) <factor> ]...
    ...
    for (;;) {
        if (lexer.peek_tok().id == token::mul) {
            ...
            auto op_tok = lexer.get_tok();
            auto rval_num = factor(lexer);
            lval_num = basic_or_vector_op([](auto lval, auto rval, const error_context& err_context) -> auto {
                if constexpr (std::is_floating_point_v<decltype(lval)> || std::is_floating_point_v<decltype(rval)>)
